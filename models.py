...
class ODEBlock(nn.Module):
    ...
    def step(self, x, dt):
        ...
        x_new = odeint(self.odefunc, x, integration_time, rtol=self.rtol, atol=self.atol)[1].cpu().detach().numpy() x_new tensor
...
class SwitchBlock(nn.Module):
    ...
    def step(self, x, dt):
        ...
        if x[ind].shape[0] > 0:   # if not empty
            x[ind] = odeint(self.odefunc1, x[ind], integration_time, rtol=self.rtol, atol=self.atol)[1] # load specific odeint

...
class Visualization:
    ...
    def compare_data(self):
        ...
        def animate(i):
            ...
            lines[trj1.shape[0]].set_data(trj1[phase1[:, i - 1] == 0, 0, i - 1],
                                          trj1[phase1[:, i - 1] == 0, 1, i - 1])  # specify data for dim 0
            lines[trj1.shape[0] + 1].set_data(trj1[phase1[:, i - 1] == 1, 0, i - 1],
                                              trj1[phase1[:, i - 1] == 1, 1, i - 1])  # specify data for dim 1
    ...
    def plot_data(self):
        ...
        def animate(i):
            ...
            lines[-2].set_data(trj[phase[:, i - 1] == 0, 0, i - 1],
                               trj[phase[:, i - 1] == 0, 1, i - 1])  # data for lines
            lines[-1].set_data(trj[phase[:, i - 1] == 1, 0, i - 1],
                               trj[phase[:, i - 1] == 1, 1, i - 1])  # data for lines
...
def get_data_loaders(data, batch_size=128, test_batch_size=128):
    ...
    train_eval_loader = DataLoader(dataset=data, batch_size=test_batch_size,
                                   shuffle=False)  # initialize train_eval_loader

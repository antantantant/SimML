...
if __name__ == '__main__':
    ...
    logger = utils.get_logger(logpath=os.path.join(args.save, 'logs'),
                              filepath=os.path.abspath(__file__))  # initialize logger, save to file
    logger.info(args)  # specify information in logger

    device = torch.device('cuda:' + str(
        args.gpu) if torch.cuda.is_available() else 'cpu')  # specify device for model (CUDA gpu if available)
    ...
    logger.info('Number of parameters: {}'.format(
        utils.count_parameters(surrogate_model)))  # store number of parameters from args
    ...
    # get data streamer
    train_loader, test_loader, train_eval_loader = utils.get_data_loaders(physics_simulation, args.batch_size,
                                                                          args.test_batch_size)  # data loaders
    ...
    lr_fn = utils.learning_rate_with_decay(
        ...
        # specify args, batches (by boundary values)
    ...
    for itr in range(args.nepochs * batches_per_epoch):
        ...
        loss = criterion(pred,
                         x[:, :, 1:].float())  # exclude initial states since they are the same for pred and target
        ...
        if itr % batches_per_epoch == 0:
            ...
            with torch.no_grad():
                train_acc = accuracy(surrogate_model, args.dt, criterion, train_eval_loader,
                                     device)  # training accuracy
    ...
    dim, time_step = physics_simulation.dim()  # specify the dimension, time step for surrogate model
